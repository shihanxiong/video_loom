...
class AudioUtils():
    ...
    def generate_mp3_from_mp4(mp4_input_filename, mp3_output_filename, ffmpeg_preset_arg="-preset ultrafast"):
        ...
        cmd = f"ffmpeg -i {FileUtils.escape_file_name(mp4_input_filename)} -vn -preset ultrafast -acodec mp3 {ffmpeg_preset_arg} {FileUtils.escape_file_name(mp3_output_filename)}"
    ...
    def generate_aac_from_mp4(mp4_input_filename, aac_output_filename, ffmpeg_preset_arg):
        cmd = f"ffmpeg -i {FileUtils.escape_file_name(mp4_input_filename)} -vn -acodec copy {ffmpeg_preset_arg} {FileUtils.escape_file_name(aac_output_filename)}"
        subprocess.check_output(cmd, shell=True)

...
# audio setting
class AudioSettingFrame(ttk.Frame, ComponentInterface):
    def __init__(self, container, **args):
        ...
        audio_setting_label = ttk.Label(self, text="Audio Settings", padding=(10))
        ...
        self.audio_track_selection = ttk.Combobox(
            self,
            width=14,
            textvariable=self.audio_track_variable,
            state="readonly",
        ...
        self.audio_track_selection.bind("<<ComboboxSelected>>", self.component_refresh)
    ...
    def component_refresh(self, _=None):
        if self.has_audio_preview():
            self.disable_button(self.generate_audio_preview_button)
            self.enable_button(self.remove_audio_preview_button)
        else:
            self.enable_button(self.generate_audio_preview_button)
            self.disable_button(self.remove_audio_preview_button)
    ...
    def generate_audio_preview(self):
        ...
        video_input = self.master.master.video_component.video_list[
